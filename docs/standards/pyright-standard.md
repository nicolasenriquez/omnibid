...
- **include**: Only check `backend/` directory
...
# Check specific file
uv run pyright backend scriptsmain.py

# Check specific directory
uv run pyright backend scriptscore/
```
...
$ uv run pyright backend scripts
backend/core/middleware.py:45:5 - error: Argument type is partially unknown
backend/models.py:12:9 - error: "dict[str, Any]" is not assignable to "MutableMapping[str, Any]"
  ...
  type-check:
    ...
    steps:
      ...
      - name: Install dependencies
        run: uv sync
      - name: Run MyPy
        run: uv run mypy backend scripts
      - name: Run Pyright
        run: uv run pyright backend scripts
```
...
uv run mypy backend scripts && uv run pyright backend scripts
...
```bash
$ time uv run pyright backend scripts
0 errors, 0 warnings, 0 informations
uv run pyright backend scripts  0.89s user 0.13s system 105% cpu 0.970 total

$ time uv run mypy backend scripts
Success: no issues found in 29 source files
uv run mypy backend scripts  2.45s user 0.21s system 101% cpu 2.622 total
```
...
uv run pyright backend scripts --verbose

# Output as JSON
uv run pyright backend scripts --outputjson
```
