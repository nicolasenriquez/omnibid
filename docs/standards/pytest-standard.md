...
- **testpaths**: Look for tests in `backend/` and `tests/` directories
...
```bash
# Run specific file
uv run pytest backend/core/tests/test_logging.py

# Run specific test
uv run pytest backend/core/tests/test_logging.py::test_function_name
