...
Normalized load (historically referenced as "Silver load") MUST upsert licitaciones, licitacion_items, ofertas, ordenes_compra, and ordenes_compra_items using explicit business conflict keys so reruns remain idempotent.
...
Normalized transform builders MUST reject records missing critical keys required to build canonical entities.
...
Normalized load MUST expose processed and rejected row outcomes per dataset so operators can diagnose data quality and ingestion drift.

#### Scenario: Monitoring normalized run outcomes
- **GIVEN** a normalized load execution for licitaciones or ordenes_compra
