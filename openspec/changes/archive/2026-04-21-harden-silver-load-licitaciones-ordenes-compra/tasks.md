...
- [x] 1.1 Confirm current Silver load baseline on one licitaciones file and one orden-compra file.
  Notes: Baseline evidence captured in `docs/evidence/silver_load_baseline_2026-04-17.md` using `202601_lic.csv` and `202604-oc.csv` with current normalized builders.
...
- [x] 2.1 Add failing tests for missing critical keys per Silver entity builder.
  Notes: Added strict oferta identity test in `tests/unit/test_normalized_transform.py` requiring supplier identity (`CodigoProveedor` or `RutProveedor`) instead of accepting only offer-name signal.
...
- [x] 2.2 Add failing tests for parsing edge cases (numeric scientific notation, sentinel dates, boolean variants).
  Notes: Added red tests for `"$1.234,56"` decimal parsing, `"verdadero"/"falso"` boolean parsing, and sentinel datetime `"1900-01-01 00:00:00"` null handling in `tests/unit/test_normalized_transform.py`.
...
- [x] 2.3 Add failing tests for idempotent upsert behavior in Silver loader helpers.
  Notes: Added `tests/unit/test_normalized_loader_helpers.py` with red fail-fast expectations for `dedupe_rows` when conflict fields are empty or business key values are missing.
  Notes: Also added a green check that latest payload wins for duplicate business keys.

## 3. Silver Hardening Implementation

- [x] 3.1 Harden transform builders in `backend/normalized/transform.py` while preserving current behavior.
...
- [x] 3.2 Harden loader behavior in `scripts/build_normalized.py` with explicit rejection accounting.
...
- [x] 4.1 Run `just test-unit` and targeted checks for Silver modules.
  ...
  - `./.venv/bin/pytest -q -m "not integration"` -> 31 passed, 1 skipped
  - `./.venv/bin/ruff check backend scripts tests` -> passed
- [x] 4.2 Run `just pipeline-normalized` against a controlled sample and verify outputs.
  Notes: Command executed with controlled sample env (`NORMALIZED_LIMIT_ROWS=2000`, `NORMALIZED_FETCH_SIZE=500`, `NORMALIZED_CHUNK_SIZE=200`) and completed successfully on 2026-04-21.
  Notes: Validation evidence captured in `docs/evidence/normalized_controlled_sample_validation_2026-04-21.md`.
  Notes: Follow-up needed: `upserted(...)` counters were negative (`-10`) due `rowcount` caveat in PostgreSQL upsert telemetry.
- [x] 4.3 Update docs/runbooks with hardened Silver behavior and operational expectations.
  Notes: Updated:
  - `docs/runbooks/local_development.md` (controlled-sample workflow + stage-gated continuation policy)
  - `docs/runbooks/operations.md` (deterministic conflict keys, fail-fast semantics, parsing behavior, telemetry caveat)
  - `docs/evidence/silver_hardening_validation_2026-04-17.md` (legacy naming note + current command mapping)
